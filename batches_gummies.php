...
if (isset($_POST['add_batch'])) {
    ...
    $query = "INSERT INTO batches_gummies (gummie_id, batch_number, batch_size, mfg_date) 
          VALUES ('{$gummie_id}', '{$batch_number}', '{$batch_size}', '{$mfg_date}')";

    if ($db->query($query)) {
    $session->msg('s', "Batch added ");
    redirect('batches_gummies.php', false);
} else {
